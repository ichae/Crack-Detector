function [eImg] = anisotropicHessian(vol, num_iter, delta_t, kappa, option, voxel_spacing)
%ANISOTROPICHESSIAN Computes the hessian based enhancement, but for an
%                   anisotropic kernel 
...
% % Suvadip: We choose the first kernel only
% option = 1;
...
for t = 1:num_iter
   ...
   %    -------------------------------------------------------------------
   %    -------------------------------------------------------------------
   %    Suvadip: We have applied anisotropic diffusion on the image. Now we
   %             do the hessian analysis for the 3D image
   %    --------------------   Hessian Analysis ---------------------------
   %    -------------------------------------------------------------------
        
        if (mod(t,2)==0)
        ImStack =[ImStack vesselness3D(diff_vol)];          
        fprintf('\nHessian Analysis done for Iteration:%d\n',t); 
        end
%         fprintf('size of imStack is :');disp(size(ImStack));
%         
%         beep on
%         beep, pause(0.2), beep
%         beep off
