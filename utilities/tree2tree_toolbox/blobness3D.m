function [ blobI ] = blobness3D(Img )

blobI=zeros(size(Img(:)));
...
%     I
    ...
    if D(sortindex(2))<0 && D(sortindex(3))<0 && D(sortindex(1))<0
        l1 = D(sortindex(1));
        l2 = D(sortindex(2));
        l3 = D(sortindex(3));
        
        R_A = abs(l1)/abs(l3);  % low for plates and filaments, high for blobs
        R_B = abs(l1)/sqrt(abs(l2)*abs(l3));    % low for filaments
        S = sqrt(l1^2+l2^2+l3^2);
        
        R_C = (abs(l1)-abs(l3))/(abs(l1)-abs(l3));
%         blobI(I) = (1 - exp(-(R_A^2)/(2*alpha^2)))*(1-exp(-(R_B^2)/(2*beta^2)))*(1-exp(-(S^2)/(2*c^2)));
blobI(I) = exp(-(R_C^2)/(2*alpha^2))*(1-exp(-(S^2)/(2*c^2)));
        if isnan(blobI(I))
            fprintf('\n NAN');
            disp([R_A R_B S]);
        end
