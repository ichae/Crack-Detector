function [phi0] = initRegionManual(I,Type,magnify)
%CREATEMASK Initialize the mask for initialization
% Left-Click after drawing once if you need  another  region
% 'ESC' to end

mask = false(size(I));
figure(1);imshow(I,'InitialMagnification',magnify); hold on;
if strcmp(Type,'rect')
...
elseif strcmp (Type,'scribble')
    
    key = -1;
    ii = 1;
    while key ~=27
        m(ii) = imfreehand('Closed',true);
        mask = mask + createMask(m(ii));
        ii = ii+1;
        [~,~,key] = ginput(1);
    end
